...
### 5. Teknik Notlar ve Mimari Kararlar

- **Mobil Uygulama (Frontend):** Projenin mobil uygulaması, cross-platform geliştirme verimliliği sağlamak amacıyla **Expo (React Native)** kullanılarak geliştirilecektir.
- **Backend Mimarisi:** Backend servisleri, **Node.js** ortamında geliştirilecek ve **MongoDB** veritabanı ile entegre çalışacaktır. Bu seçim, esnek veri modelleme ve JavaScript ekosistemiyle tam uyum avantajları sunmaktadır.
- **Veritabanı:** NoSQL yapısının esnekliği nedeniyle **MongoDB** tercih edilmiştir.
- **Hariç Tutulan Teknolojiler:** Projede **Supabase** gibi Backend as a Service (BaaS) platformları kullanılmayacaktır. Tüm backend mantığı, özel olarak geliştirilecek olan API üzerinde kurgulanacaktır.
- **Geliştirme Yaklaşımı:** Mobil uygulama geliştirmesi, backend'den bağımsız ilerlemek ve UI/UX'i hızla tamamlamak için ilk aşamada **mock datalar** ile yapılacaktır. Backend API'si hazır olduğunda entegrasyon sağlanacaktır.